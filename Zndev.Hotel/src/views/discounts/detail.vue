...
<div>	
	<div class="main-container" ref="scrollBox">
		...
		<div class="nav-header" :class="{'transparent':scrollPosition<100}" v-if="isLoadComplete">
			<x-header :title="scrollPosition<100?'':'抢购详情'">
			</x-header>
		</div>
				...
				<collapsible>
					...
					<div>
						 <group label-align="right" label-width="5em" class="no-border">
						 	...
						 	<cell primary="content" title="抢购价格" :value="`￥ ${model.priceNow}/${model.unit}`"></cell>
						 	...
						 	<cell primary="content" title="预约时间"  v-if="!model.noAppointment">{{model.appointmentBeginTime|dateFormat('YY-MM-DD HH:mm:ss')}}至{{model.appointmentBeginTime|dateFormat('YY-MM-DD HH:mm:ss')}}</cell>
	   				        ...
	   				        <cell primary="content" title="房态先知"  v-if="model.category==1&&model.roomId>0&&model.priceTypeId>0">
	   				        	<div>想提前知道哪天有房可预约吗？</div>
	   				        	<x-button slot="child" mini type="primary" :gradients="['#FF61AD','#FF2719']" @click.native="viewRoomState(model)">立即点击</x-button>
	   				        </cell>
					    </group>
					</div>
				</collapsible>
		...
		<div class="buy-bar fixed-bottom" v-if="isLoadComplete">
			<flexbox  class="content-wrap" :gutter="0" v-if="statusInfo.status==1">
		      <flexbox-item :span="9">
		      	<div class="time">
	      			<p class="tips">{{model.beginTime}}开售</p>
	      			<div class="c">
	      				<template v-if="countDonw.stop">
	      					<div class="over-time">该产品已抢购结束，敬请关注其他产品喔</div>
	      				</template>
	      				<template v-else>
		      				剩
		      ...
		      <flexbox-item>
		      	<div class="buy-btn" :class="{'disabled':countDonw.stop}" @click="toBook(model.tid)">
		      		{{countDonw.stop?'已经结束':'立即抢购'}}
		      	</div>
		      </flexbox-item>
		    </flexbox>
		    <div class="wait-begin content-wrap" v-else>
	    		<span class="iconfont icon-naozhong"></span>距离开抢时间还有
  				<template v-if="countDonw.d>0">
  					<span>{{countDonw.d}}</span>天
  				</template>
  				<span>{{countDonw.h}}</span>小时
...
export default {
  ...
  components: {
    ...
    book: () => import(/* webpackChunkName: "discounts" */ "./book.vue"),
    roomState: () =>
