			...
			<div class="content-wrap top-padding-filter-bar">
				...
				<div v-if="!isLoading" v-sticky="{
					...
					<tab :line-width=1 active-color='#f39709' v-model="curTabType">
						<tab-item class="vux-center"  v-for="(item, index) in tabs" :key="index" @on-item-click="tabChange">{{item.title}}</tab-item>
					</tab>
					<!--<tab :scroll-threshold="5" :line-width="0" class="filter-tabs">
					  <tab-item :selected="true">
					  	<span>全部</span>	
					  </tab-item>
				      <tab-item v-for="n in 5" :key="n"><span>已发货{{ n }}</span></tab-item>
				    </tab>-->
				</div>
				<template v-if="!isLoading">
					<msg class="empty-tips-msg" v-if="isNoData" title="暂无相应的抢购活动" description="我们将为您推出更为优惠的活动，敬请期待！" icon="info"></msg>
					<product-list v-else class="special">
						<div slot="header" class="title">
							{{!isFilter?'全部':'筛选结果'}}
						</div>
						<!--<div slot="footer">加载更多</div>-->
						<div class="product" v-for="(p,index) in products" :key="index">
							<div class="img-bd">
								<!--<div class="pro-status">
									{{p.status==1?'抢购中':'未开始'}}
								</div>-->
								<img @click.prevent="viewDetail(p.tid)" v-lazy="_getImgUrl(p.snapShot)"  :key="p.snapShot" :alt="p.Name">
								<div :class="'type '+'type-'+p.category">
									{{p.categoryLabel}}
								</div>
								<div  class="time">
									<span class="iconfont icon-naozhong"></span>
									<strong>{{p.beginTime}} 开抢</strong>
								</div>
							</div>
							<div class="tags">
	...
	export default {
		...
		computed:{
			emptyTips:function(){
				...
				if(this.isNoData){
					return '暂无相应的抢购活动，敬请期待';
//					if(this.curTabType==0)
//						return '暂无未开始的抢购，敬请期待';
//					return '暂无抢购中的抢购，敬请期待';
				}
				return '';
			}
		},
