							...
							<div class="order-item" v-for="(order,i) in orders" @click="showDetail(order.tid)" :key="i">
								<div class="hd">
									<div class="float-left">
										<span class="iconfont icon-canyin"></span>{{order.storeName}}
									</div>
									<div class="float-right status">
										{{toStatusShow(order.status,order.logisticsType)}}
									</div>
								</div>
								<div class="bd border-0">
									<div class="bold">
										<div class="float-left">
											{{order.logisticsType==0?'到店消费':'外卖派送'}}
										</div>
									...
									<div class="time">
										{{order.insertTime | dateFormat('YYYY年MM月DD日')}}
									</div>
								</div>
								<!--<div class="bar">
...
export default {
  ...
  components: {
    detail: () =>
      import(/* webpackChunkName: "myDiningOrder" */ "./repast_detail.vue")
  },
  methods: {
    ...
    toStatusShow(status, logisticsType) {
      if (status == 0) return "待付款";
      if (status == 1) return "待消费";
      if (status == 2) return logisticsType == 0 ? "已消费" : "已发货";
      if (status == 3) return logisticsType == 0 ? "已离店" : "已签收";
